...
if ($conn->connect_error) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $nameMakeup = $_POST["nameMakeup"];
        $dureeMakeup = $_POST["dureeMakeup"];
        $prixMakeup = $_POST["prixMakeup"];


        // Check if all required fields are provided
        if ($nameMakeup == null || $prixMakeup == null || $dureeMakeup == null) {
            echo 'err';
        } else {

            // Perform the database insertion
            // Préparer la requête SQL INSERT avec des paramètres sous forme de marqueurs de position (?)
            $sql = "INSERT INTO maquillage (name, duree, prix) VALUES (?, ?, ?)";
            ...
            if ($stmt) {
                ...
                $stmt->bind_param("sss", $nameMakeup, $dureeMakeup, $prixMakeup);
